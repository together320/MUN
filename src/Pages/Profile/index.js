...
function LoanItem({item, preRenderLoan}) {
    ...
    const initialize = async () => {
        ...
        await getNFTInfoByMintAddress(item.account.nftMint.toBase58()).then(async (res) => {
            ...
            setRepayAmount(Math.fround(item.account.requestAmount.toNumber() / LAMPORTS_PER_SOL * (Math.pow(1 + item.account.interest.toNumber() / 10000, item.account.duration.toNumber()))).toFixed(2));
...
function OfferItem({item, preRenderOffer}) {
    ...
    const initialize = async () => {
        ...
        await getNFTInfoByMintAddress(item.account.nftMint.toBase58()).then(async (res) => {
            ...
            setInterest(Math.fround(item.account.requestAmount.toNumber() / LAMPORTS_PER_SOL * (Math.pow(1 + item.account.interest.toNumber() / 10000, item.account.duration.toNumber()) - 1)).toFixed(2));
            setInterestApy(Math.fround(Math.pow(1 + item.account.interest.toNumber() / 10000, 365) - 1).toFixed(0));
...
export default function Profile() {
    ...
    const preRenderLoan = async () => {
        ...
        try {
            ...
            let aContracts = await ownerContracts.sort(function(a, b) {
                if (a.account.orderStatus && !b.account.orderStatus) {
                    return -1; // 'a' comes first
                  } else if (!a.account.orderStatus && b.account.orderStatus) {
                    return 1; // 'b' comes first
                  } 
                  return 0; // no change in order
            });
            setLoanList(aContracts);
        } catch(e){
            diagCtx.showError(e.message);
            diagCtx.hideLoading();
            return;
    ...
    const renderOverview = () => {
        return <>
            <Box className="lg:grid gap-[20px] 2xl:gap-[25px] grid-cols-1 sm:grid-cols-3" gridTemplateColumns={'1fr 1fr 1fr'}>
                ...
                <Box className="flex flex-col bg-[#1B1E3D] rounded-[12px] p-[35px] justify-center mb-[10px]"
                style={{ backgroundImage: `url(/logo_small.png)`, backgroundRepeat : 'no-repeat', backgroundSize : '39px 16px', backgroundPosition : '10px 10px' }}>
                    <Box className="flex justify-center mb-[35px]">
