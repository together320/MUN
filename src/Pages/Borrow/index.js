...
function BorrowItem({ collectionSymbol, nftList, getCollectionOffers }) {
    ...
    const [lowInterest, setLowInterest] = useState(0);
    const [midInterest, setMidInterest] = useState(0);
    const [highInterest, setHighInterest] = useState(0);

    const [bestOffer, setBestOffer] = useState(0);
    const [interest, setInterest] = useState([false, false, false]);
    const [duration, setDuration] = useState([false, false, false, false]);
    ...
    useEffect(() => {
        ...
        for(let i = 0; i < offerList.length; i++){
            if(offerList[i].account.depositAmount / LAMPORTS_PER_SOL >= floorPrice * (offerList[i].account.percentFloorPrice.toNumber()) * 1.0 / 100)
                if((floorPrice * (offerList[i].account.percentFloorPrice.toNumber()) * 1.0 / 100) > temp)
                    temp = (floorPrice * (offerList[i].account.percentFloorPrice.toNumber()) * 1.0 / 100);
        }
        setBestOffer(temp);
        let a = false, b = false, c = false, d = false;
        for(let i = 0; i < offerList.length; i++){
            if(offerList[i].account.depositAmount / LAMPORTS_PER_SOL >= floorPrice * (offerList[i].account.percentFloorPrice.toNumber()) * 1.0 / 100){
                if(offerList[i].account.interestAmount.toNumber() <= 30) a = true;
                if(offerList[i].account.interestAmount.toNumber() > 30 && offerList[i].account.interestAmount.toNumber() < 70) b = true;
                if(offerList[i].account.interestAmount.toNumber() >= 70) c = true;
            }
        }
        setInterest([a, b, c]);
        for(let i = 0; i < offerList.length; i++){
            if(offerList[i].account.depositAmount / LAMPORTS_PER_SOL >= floorPrice * (offerList[i].account.percentFloorPrice.toNumber()) * 1.0 / 100){
                if(checkDuration(1, offerList[i].account.duration.toNumber())) a = true;
                if(checkDuration(7, offerList[i].account.duration.toNumber())) b = true;
                if(checkDuration(14, offerList[i].account.duration.toNumber())) c = true;
                if(checkDuration(30, offerList[i].account.duration.toNumber())) d = true;
            }
        }
        setDuration([a, b, c, d]);
    }, [offerList, floorPrice]);
...
//        const imageUrl = nft.json.image;
        ...
        try {
            ...
            let ownerPools = await pools.filter(function(item){
                return item.account.collections.includes(collectionSymbol) && item.account.poolStatus === true && item.account.isPaused === false;
            });
    ...
    if (isDesktop) {
        return <Box className="mb-[16px]">
            <Box className={`px-[26px] py-[18px] bg-[#111430] ${classes} grid gap-[5px] 2xl:gap-[10px]`} gridTemplateColumns={'80px 96px 30fr 20fr 40fr 40fr 20fr'} style={{ minWidth: 'fit-content' }}>
                ...
                <SolanaItem value={bestOffer.toFixed(2)} style={{ minWidth: '70px' }} />
                <Box className="flex justify-center items-center">
                    <InterestButton className={`${interest[0] === true ? "bg-[#38D39C]" : "bg-[#1B1E3D]"} w-[32px] h-[32px] flex items-center justify-center mr-[5px]`}>L</InterestButton>
                    <InterestButton className={`${interest[1] === true ? "bg-[#FFBE5C]" : "bg-[#1B1E3D]"} w-[32px] h-[32px] flex items-center justify-center mr-[5px]`}>M</InterestButton>
                    <InterestButton className={`${interest[2] === true ? "bg-[#EB5757]" : "bg-[#1B1E3D]"} w-[32px] h-[32px] flex items-center justify-center mr-[5px]`}>H</InterestButton>
                </Box>
                <Box className="flex justify-center items-center">
                    <InterestButton className={`bg-[#1B1E3D] w-[32px] h-[32px] flex items-center justify-center mr-[5px] ${duration[0] === true ? "!text-[#5C84FF]" : "!text-[#666880]"}`}>1</InterestButton>
                    <InterestButton className={`bg-[#1B1E3D] w-[32px] h-[32px] flex items-center justify-center mr-[5px] ${duration[1] === true ? "!text-[#5C84FF]" : "!text-[#666880]"}`}>7</InterestButton>
                    <InterestButton className={`bg-[#1B1E3D] w-[32px] h-[32px] flex items-center justify-center mr-[5px] ${duration[2] === true ? "!text-[#5C84FF]" : "!text-[#666880]"}`}>14</InterestButton>
                    <InterestButton className={`bg-[#1B1E3D] w-[32px] h-[32px] flex items-center justify-center mr-[5px] ${duration[3] === true ? "!text-[#5C84FF]" : "!text-[#666880]"}`}>30</InterestButton>
                </Box>
                            ...
                            offerList.map((offer, key) => {
                                ...
                                onClick={() => setSelectedOffer(key)}>
                                    ...
                                    <Box className="flex items-center justify-center">
                                        <InterestText color={`${offer.account.interestAmount <= 30 ? '#38D39C' : (offer.account.interestAmount > 30 && offer.account.interestAmount < 70) ? '#FFBE5C' : '#EB5757'}`}
                                        value={offer.account.interestAmount / 100} />
                                        <CollectionCashText className="ml-[5px] my-auto cursor-pointer" style={{ color: '#494D73' }}>
                                            <InfoNotify content={`Total Interest is ${offer.account.interestAmount / 100}% or ${Math.fround(floorPrice * (offer.account.percentFloorPrice.toNumber()) * 1.0 / 100 * (Math.pow(1 + (offer.account.interestAmount / 10000), offer.account.duration.toNumber()) - 1)).toFixed(2)} SOL`} />
                                        </CollectionCashText>
                                    </Box>
                                    <Box className="flex items-center justify-center">
